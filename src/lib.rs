...
impl DirDecoder {
    ...
    pub fn new_from_file_name(name: &str) -> io::Result<Self> {
        if name.ends_with(DirDecoder::Zip.suffix()) {
            Ok(DirDecoder::Zip)
...
pub fn unpack_directory(
    ...
    // ref. https://stackoverflow.com/questions/48225618/ls-permission-denied-even-though-i-have-read-access-to-the-directory/48225881#48225881
    fs::set_permissions(target_dir, PermissionsExt::from_mode(0o775))?;
    ...
    let decompressed_path = {
        if src_archive_path.ends_with(dec.compression_ext()) {
            ...
            format!("{}.decompressed", src_archive_path)
        }
    };
        ...
        DirDecoder::Zip => {
            ...
            let mut zip = match ZipArchive::new(zip_file) {
                ...
                Err(e) => {
                    return Err(Error::new(
                        ErrorKind::Other,
                        format!("failed ZipArchive::new on {} ({})", src_archive_path, e),
                    ));
                }
            };
        ...
        DirDecoder::ZipGzip => {
            ...
            let mut zip = match ZipArchive::new(zip_file) {
                ...
                Err(e) => {
                    return Err(Error::new(
                        ErrorKind::Other,
                        format!("failed ZipArchive::new on {} ({})", decompressed_path, e),
                    ));
                }
            };
            for i in 0..zip.len() {
...
fn test_pack_unpack() {
    ...
    for (i, _) in encs.iter().enumerate() {
        let encoder = Encoder::new(encs[i]).unwrap();
        let decoder = Decoder::new(decs[i]).unwrap();

        let packed = pack(&contents, encoder).unwrap();
        ...
        let unpacked = unpack(&packed, decoder).unwrap();
    ...
    for (i, _) in encs.iter().enumerate() {
        ...
        pack_file(&orig_path, packed_path, encoder).unwrap();
        ...
        unpack_file(packed_path, unpacked_path, decoder).unwrap();
    ...
    let decs = vec![
        "zip",      // zip decoder has no level
        "tar-gzip", // gzip decoder has no level
        "zip-gzip", // gzip decoder has no level
        "tar-zstd", // zstd decoder has no level
        "tar-zstd", // zstd decoder has no level
        "tar-zstd", // zstd decoder has no level
        "zip-zstd", // zstd decoder has no level
        "zip-zstd", // zstd decoder has no level
        "zip-zstd", // zstd decoder has no level
    ];
    for (i, _) in encs.iter().enumerate() {
        ...
        if encs[i].ne("zip") {
            ...
            let meta_packed = fs::metadata(&compressed_path).unwrap();
    ...
    fs::remove_dir_all(src_dir_path).unwrap();
}
